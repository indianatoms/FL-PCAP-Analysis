...
class Fedavg:
    ...
    def client_login(self, connection):
        ...
        password = hashlib.sha256(
            str.encode(password)
        ).hexdigest()  # Password hash using SHA256
        # REGISTERATION PHASE
...
if __name__ == "__main__":
    ...
    while True:
        ...
        client_handler = threading.Thread(target=fedavg.client_login, args=(Client,))
    ...
    for round in range(NUMBER_OF_ROUNDS):
        ...
        for conn in fedavg.connections:
            client_handler = threading.Thread(target=fedavg.wait_for_data, args=(conn,))
            client_handler.start()
            threads.append(client_handler)
        ...
        applicable_clients = (
            fedavg.clients
        )  # random.sample(fedavg.clients,len(fedavg.clients))
        ...
        for conn in fedavg.connections:
            client_handler = threading.Thread(
                target=fedavg.send_request, args=(conn, fedavg.model)
            )
            client_handler.start()
            threads.append(client_handler)
            if len(threads) == NUMBER_OF_CLIENTS:
