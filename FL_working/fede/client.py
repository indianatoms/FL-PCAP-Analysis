...
class Client:
    ...
    def init_empty_model(self, epochs=20):
        if self.model_name == Supported_modles.SGD_classifier:
            self.model = SGDClassifier(
                loss="log", alpha=0.001, penalty="l1", max_iter=epochs
            )
        if self.model_name == Supported_modles.logistic_regression:
            self.model = LogisticRegression(C=100000, penalty="l2")
    ...
    def fed_avg_prepare_data(self, epochs):
        ...
        fed = Fed_Avg_Client(self.name, dataset_size, self.model)
        return fed
    ...
    def get_global_model(self):
        ...
        if (
            self.model_name == Supported_modles.SGD_classifier
            or self.model_name == Supported_modles.logistic_regression
        ):
    ...
    def calcualte_global_model(self):
        ...
        api_url = (
            "http://" + str(HOST) + ":" + str(PORT) + "/model/global/calculate/fedavg"
        )
        json_data = {"models_id": [1, 2, 3]}
    ...
    def send_local_model(self, fed):
        ...
        if (
            ...
            json_data = {
                "type": self.model_name.value,
                "model": {
                    "intercept": fed.model.intercept_[0],
                    "coefs": fed.model.coef_[0].tolist(),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--port", dest="port", type=int, help="port on which socket will run"
    )
    parser.add_argument(
        "--address", dest="address", type=str, help="ip on which socket will run"
    )
    parser.add_argument("--data", dest="data", type=str, help="path to data")
    parser.add_argument(
        "--conn", dest="conn", type=str, help="Use either socket or api"
    )
    parser.add_argument(
        "--model", dest="model", type=str, help="Specify model type: NN, LR, SGD"
    )
